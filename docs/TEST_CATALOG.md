...
| TEST-LIVE-001 | Authenticated telemetry is valid, fresh, ordered, rate-bounded, newer-wins, and attached to the enrolled device's correct active trip | Domain, persistence, realtime integration | Domain policy tests, PostgreSQL tests, simulator and load suite | in progress |
...
| TEST-LIVE-003 | Owners, explicitly granted admins, branch managers, dispatchers, drivers, and conductors see only their organization/branch/route/trip scope | Authorization, API, persistence, browser journey | Role/capability matrix, cross-tenant and wrong-branch suite, staff browser suite | in progress |
