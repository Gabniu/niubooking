...
- [ ] NIU Driver native runtime: wire the native OIDC redirect, OS secure storage, token refresh policy, background location task, and Traccar Client SDK only in that app layer; then pass physical-device background tests.
