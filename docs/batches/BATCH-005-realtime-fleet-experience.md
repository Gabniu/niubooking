...
- [x] Native auth boundary slice: the platform-neutral driver core now defines secure-storage adapters, expiry-aware restore/sign-out, and token-free snapshots for the future NOVA OIDC native client; provider refresh remains an app-edge concern.
- [x] Durable telemetry queue slice: native apps can provide a local persistence adapter that rehydrates ordered positions after restart, serializes concurrent mutations, and fails closed on corrupt records without changing the server source of truth.
- [x] Driver app orchestration slice: the shared core now coordinates auth restore, active-session recovery, duplicate-start protection, assigned-trip start/stop, telemetry/offline states, and safe sign-out without exposing session identifiers to the UI.
- [x] NIU Driver first-app shell: an Expo SDK 57 React Native surface now uses the compact Booking design tokens, Hanken Grotesk, truthful setup-pending state, sign-in/permission/start-sharing explanation, and no invented trip data. Native OIDC, secure storage, and telemetry actions remain explicit adapters rather than fake UI.
- [x] NIU Driver secure-storage boundary: the app now validates and stores native auth credentials and active-session metadata through OS-backed SecureStore adapters, failing closed on malformed records; queue persistence and OIDC/PKCE exchange remain separate follow-up adapters.
- [x] NIU Driver OIDC edge adapter: the Expo app discovers the configured issuer, requires HTTPS endpoints and S256 PKCE, completes the browser code exchange, and persists only the expiring access credential by default; builds with refresh enabled request `offline_access`, keep the refresh credential in a separate SecureStore key, rotate it when returned, and clear both credentials after `invalid_grant`.
- [x] NIU Driver background location boundary: a top-level Expo TaskManager task and explicit permission/start/stop adapter restore an active server session on cold start, convert valid OS samples into the shared telemetry contract, persist offline samples through the app queue adapter, and stop capture when the session is absent or blocked; physical Android/iOS proof remains required.
- [x] NIU Driver Traccar boundary: the native app exposes validated `createNativeTraccarClient()` and session `createNativeTraccarCapture()` adapters for the official SDK; authorized session start/handover issues a hashed, revocable `niu_traccar_v1` credential, and the API normalizes OsmAnd query/form payloads into the existing persistence path with server-derived session scope and sequence ordering.
...
- [ ] NIU Driver native runtime: connect assigned-device provisioning and the visible driver start/stop journey to the session capture adapter, complete native SDK storage review, and pass physical-device background tests.
