...
| CAP-GTFS-002 | GTFS-Realtime publication | Transit interoperability | Transport | in progress | Privacy-safe protobuf VehiclePositions from fresh expiring telemetry, authoritative TripUpdates from published patterns and persisted stop-times, and owner/admin Alert lifecycle controls tied to the active Schedule; bounded worker refresh, short-lived tenant-safe caches, validators, request-time fallback, and the admitted `/app/gtfs` Alert workspace with optional route/stop/trip targeting are wired, while occupancy and detours remain | TEST-GTFS-002 | BATCH-005 |
