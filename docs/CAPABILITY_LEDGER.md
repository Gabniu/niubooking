...
| CAP-GTFS-001 | GTFS Schedule publication | Transit interoperability | Transport; Charter | in progress | Stable public IDs, validated Schedule draft rules, deterministic core GTFS text serialization, independent file-level reference validation, deterministic ZIP artifact creation, immutable content-addressed filesystem writes, tenant-safe active-version lookup, cacheable public Schedule delivery, and audited owner/admin publish, withdraw, and rollback commands with idempotency; database-coordinated artifact promotion and full exporter remain | TEST-GTFS-001 | BATCH-005 |
