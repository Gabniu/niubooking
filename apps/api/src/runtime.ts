...
export function readApiRuntimeConfig(env: Record<string, string | undefined>): ApiRuntimeConfig {
  ...
  const gtfsArtifactDir = env.GTFS_ARTIFACT_DIR?.trim();
  return { databaseUrl, bookingHoldSecret, ...(gtfsArtifactDir ? { gtfsArtifactDir } : {}) };
}
