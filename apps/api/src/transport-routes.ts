...
export interface TransportAdmin {
  ...
  issuePublicLiveSession?(ticketToken: string): Promise<{ viewerToken: string; expiresAt: Date } | null>;
  readPublicLiveViewer?(viewerToken: string): Promise<{ tenantId: string; projection: RiderLiveTripProjection } | null>;
...
function riderResponse(value: { tenantId: string; projection: RiderLiveTripProjection } | null) { return value ? { data: value.projection, error: null } : { data: null, error: { code: "LIVE_TRIP_UNAVAILABLE" as const, message: "Live trip location is not available." } }; }
...
export function registerTransportRoutes(app: FastifyInstance, dependencies: TransportRouteDependencies): void {
  ...
  app.post<{ Params: { token: string } }>("/v1/public/transport/tickets/:token/live-session", async (request, reply) => {
    if (!dependencies.transportAdmin?.issuePublicLiveSession) return reply.code(503).send({ data: null, error: { code: "LIVE_TRIP_UNAVAILABLE", message: "Live trip tracking is temporarily unavailable." } });
    ...
    const session = await dependencies.transportAdmin.issuePublicLiveSession(request.params.token);
    return session ? reply.send({ data: { viewerToken: session.viewerToken, expiresAt: session.expiresAt.toISOString() }, error: null }) : reply.code(404).send({ data: null, error: { code: "TRACKING_LINK_INVALID", message: "This live trip link is not available." } });
  });

  app.get<{ Params: { viewerToken: string } }>("/v1/public/transport/live/:viewerToken", async (request, reply) => {
    if (!dependencies.transportAdmin?.readPublicLiveViewer) return reply.code(503).send({ data: null, error: { code: "LIVE_TRIP_UNAVAILABLE", message: "Live trip location is temporarily unavailable." } });
    if (!/^[A-Za-z0-9_-]{32,256}$/u.test(request.params.viewerToken)) return reply.code(404).send({ data: null, error: { code: "TRACKING_LINK_INVALID", message: "This live trip link is not available." } });
    const value = await dependencies.transportAdmin.readPublicLiveViewer(request.params.viewerToken);
  ...
  app.get<{ Params: { viewerToken: string } }>("/v1/public/transport/live/:viewerToken/stream", async (request, reply) => {
    if (!dependencies.transportAdmin?.readPublicLiveViewer || !dependencies.liveStream) return reply.code(503).send({ data: null, error: { code: "LIVE_TRIP_UNAVAILABLE", message: "Live trip updates are temporarily unavailable." } });
    if (!/^[A-Za-z0-9_-]{32,256}$/u.test(request.params.viewerToken)) return reply.code(404).send({ data: null, error: { code: "TRACKING_LINK_INVALID", message: "This live trip link is not available." } });
    const initial = await dependencies.transportAdmin.readPublicLiveViewer(request.params.viewerToken);
    ...
    const snapshot = async (type: RiderLiveStreamEvent["type"]): Promise<void> => { if (writing) { queued = true; return; } writing = true; try { const value = await dependencies.transportAdmin!.readPublicLiveViewer!(request.params.viewerToken); if (!value) { write(riderStreamEvent("snapshot", ++version, riderResponse(null))); raw.end(); return; } write(riderStreamEvent(type, ++version, riderResponse(value))); } finally { writing = false; if (queued && !raw.destroyed) { queued = false; void snapshot("changed"); } } };
