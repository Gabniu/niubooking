...
export function registerGtfsRoutes(app: FastifyInstance, dependencies: RouteDependencies): void {
  ...
  app.get<{ Params: { publicSlug: string } }>("/v1/public/gtfs/:publicSlug/alerts.pb", async (request, reply) => {
    if (!dependencies.gtfsPublication?.readPublicAlerts) return reply.code(503).send({ data: null, error: { code: "GTFS_REALTIME_UNAVAILABLE", message: "Alerts are not available for this feed yet." } });
    try {
      const feed = await dependencies.gtfsPublication.readPublicAlerts(request.params.publicSlug);
      if (!feed) return reply.code(404).send({ data: null, error: { code: "GTFS_REALTIME_NOT_FOUND", message: "Alerts are not enabled for this feed." } });
      const payload = Buffer.from(serializeGtfsRealtimeAlerts(feed)); const etag = `"${createHash("sha256").update(payload).digest("hex")}"`;
      if (request.headers["if-none-match"] === etag) return reply.code(304).header("ETag", etag).send();
