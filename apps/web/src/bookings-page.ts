...
function customerLabel(customerId: string): string { const option = [...customerElement.options].find((item) => item.value === customerId); return option?.textContent?.trim() || "Customer name unavailable"; }
function render(bookings: readonly BookingSummary[]): void {
  ...
  for (const booking of bookings) {
    ...
    const meta = document.createElement("span"); meta.textContent = `${dateLabel(booking.startsAt)} · ${customerLabel(booking.customerId)}`;
