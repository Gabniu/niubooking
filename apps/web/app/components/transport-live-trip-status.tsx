...
export function TransportLiveTripStatus({ token, apiBase }: { token: string; apiBase: string }) {
  ...
  return <section className="transport-live-trip" aria-labelledby="live-trip-title"><header><div><p className="public-eyebrow">LIVE TRIP</p><h2 id="live-trip-title">Vehicle location</h2></div>{value && <span className={`transport-live-badge transport-live-${value.freshness}`}>{freshnessLabels[value.freshness]}</span>}</header>{value && <div className="transport-live-details"><p>{value.latitude === null ? "Your operator has not shared a location yet." : "Your vehicle is sharing its latest location."}</p><dl><div><dt>Last update</dt><dd>{dateText(value.capturedAt)}</dd></div><div><dt>Connection</dt><dd>{streaming ? "Live updates" : "Checking periodically"}</dd></div></dl></div>}{state.kind !== "ready" && <p className="transport-live-message" role="status" aria-live="polite">{state.message}</p>}{state.kind !== "ready" && <button className="transport-secondary transport-live-retry" type="button" onClick={() => setRetryKey((current) => current + 1)}>Try again</button>}<small>Map view will appear when route geometry is ready. This status never guesses a vehicle position.</small></section>;
}
