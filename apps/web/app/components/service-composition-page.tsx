...
import {
  createRequirement, createVariant, fetchRequirements, fetchVariants,
  setRequirementStatus, setVariantStatus,
...
type PendingAction = "variant" | "requirement" | "availability" | `variant-status:${string}` | `requirement-status:${string}` | null;
type Availability = { slots: readonly { startsAt: string; endsAt: string; assignments: readonly { resourceIds: readonly string[]; requirementLabel?: string }[] }[]; rejected: readonly { requirementId: string; reason: string }[] };
...
export function ServiceCompositionPage() {
  ...
  const [availability, setAvailability] = useState<DataState<Availability>>({ kind: "idle" });
  ...
  const loadRequirements = (variantId = selectedVariantId) => {
    ...
    void fetchRequirements(request, apiBase, admission.tenantId, serviceId, variantId || undefined)
      .then(setRequirements).catch(() => setRequirements({ kind: "error", message: "Requirements could not be loaded." }));
  };
  const load = () => {
    if (admission.kind !== "ready" || !serviceId) return;
    setVariants({ kind: "loading" });
    void fetchVariants(request, apiBase, admission.tenantId, serviceId).then((result) => {
      setVariants(result);
  ...
  async function saveVariant(event: FormEvent<HTMLFormElement>) {
    ...
    if (admission.kind !== "ready" || !serviceId || pendingAction) return;
    ...
    if (duration !== null && (!Number.isInteger(duration) || duration < 5 || duration > 1440)) return setMessage("Duration must be a whole number from 5 to 1,440 minutes.");
    setPendingAction("variant");
    try {
      const result = await createVariant(request, apiBase, admission.tenantId, serviceId, { name: variant.name.trim(), durationMinutes: duration });
      if (result.kind !== "ready") return setMessage(result.message);
      setVariant({ name: "", durationMinutes: "" }); setMessage(`${result.data.name} was added.`); load();
    } catch { setMessage("Variant could not be added. Please try again."); } finally { setPendingAction(null); }
  }
