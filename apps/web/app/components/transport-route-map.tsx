...
function pointsFromStops(stops: readonly PublicTransportStopSummary[]): Point[] {
  return stops.flatMap((stop) => stop.latitude !== undefined && stop.longitude !== undefined ? [[stop.longitude, stop.latitude] as const] : []);
}

function project(point: Point, bounds: { minX: number; maxX: number; minY: number; maxY: number }): Point {
  const x = bounds.maxX === bounds.minX ? 50 : 8 + ((point[0] - bounds.minX) / (bounds.maxX - bounds.minX)) * 84;
  const y = bounds.maxY === bounds.minY ? 50 : 92 - ((point[1] - bounds.minY) / (bounds.maxY - bounds.minY)) * 84;
  return [x, y];
}

function boundsFor(points: readonly Point[]): { minX: number; maxX: number; minY: number; maxY: number } {
  const xs = points.map(([x]) => x);
  const ys = points.map(([, y]) => y);
  return { minX: Math.min(...xs), maxX: Math.max(...xs), minY: Math.min(...ys), maxY: Math.max(...ys) };
}

export function TransportRouteMap({ geometry, stops, livePosition, label = "Route map", smoothLivePosition = false }: { geometry?: PublicTransportRouteGeometry | null | undefined; stops: readonly PublicTransportStopSummary[]; livePosition?: LivePosition; label?: string; smoothLivePosition?: boolean }) {
  ...
  const projectedLive = smoothed ? project([smoothed.longitude, smoothed.latitude], bounds) : null;
