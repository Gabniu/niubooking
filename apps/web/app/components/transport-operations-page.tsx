...
import { FleetOperationsPanel } from "./fleet-operations-panel.js";
...
const request = (input: string, init: { credentials: "include"; method?: "POST" | "PUT"; headers?: Record<string, string>; body?: string }) => window.fetch(input, init);
...
export function TransportOperationsPage() {
  ...
  async function selectTrip(tripId: string) { setSelectedTripId(tripId); if (admission.kind !== "ready") return; setManifest({ kind: "loading", value: [] }); try { const result = await fetchTransportManifest(request, apiBase, admission.tenantId, tripId); setManifest(result.kind === "ready" ? result : { ...result, value: [] }); if (result.kind === "ready") { try { const customersState = await fetchCustomers(request, apiBase, admission.tenantId); setCustomerNames(customersState.kind === "ready" ? Object.fromEntries(customersState.customers.map((customer) => [customer.id, customer.displayName])) : {}); } catch { setCustomerNames({}); } } } catch { setManifest({ kind: "error", value: [], message: "The trip manifest could not be loaded. Check your connection and try again." }); } }
