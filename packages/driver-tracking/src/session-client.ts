...
export interface DriverSessionFetcher {
  (url: string, init: { credentials: "include"; method: "POST"; headers: { "content-type": "application/json"; authorization?: string }; body: string }): Promise<{ status: number; json(): Promise<unknown> }>;
}
...
export function createDriverSessionClient(fetcher: DriverSessionFetcher, baseUrl: string, accessToken?: string) {
  const headers = { "content-type": "application/json" as const, ...(accessToken ? { authorization: `Bearer ${accessToken}` } : {}) };
  return {
    ...
    async end(tenantId: string, sessionId: string, reason = "stopped from NIU Driver"): Promise<DriverSessionEndState> {
      const response = await fetcher(`${baseUrl}/v1/tenants/${encodeURIComponent(tenantId)}/fleet/tracking-sessions/${encodeURIComponent(sessionId)}/end`, { credentials: "include", method: "POST", headers, body: JSON.stringify({ reason }) });
