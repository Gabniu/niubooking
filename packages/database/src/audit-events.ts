...
import { validateAuditMetadata, type AuditAction, type AuditActorType, type AuditEntityType, type AuditEvent } from "@bookingapp/domain";
import type { SqlExecutor } from "./tenant-membership.js";

interface AuditRow { id: string; tenant_id: string; actor_type: AuditActorType; actor_id: string | null; action: AuditAction; entity_type: AuditEntityType; entity_id: string; metadata: Readonly<Record<string, string | number | boolean | null>>; occurred_at: Date; }
...
export async function appendAuditEvent(executor: SqlExecutor, input: { tenantId: string; actorType: AuditActorType; actorId: string | null; action: AuditAction; entityType: AuditEntityType; entityId: string; metadata: Readonly<Record<string, string | number | boolean | null>> }): Promise<AuditEvent> {
