...
import { classifyPositionFreshness, isCapacityReserving, validateReservationStatusChange, validateTransportPassengerReservationDraft, validateTransportRouteDraft, validateTransportSeatAssignment, validateTransportTicketDraft, validateTransportTripDraft, type PublicTransportTicket, type PublicTransportTrip, type ReservationStatus, type TransportBoarding, type TransportManifestEntry, type TransportPassengerReservation, type TransportPassengerReservationDraft, type TransportRoute, type TransportRouteDraft, type TransportTicket, type TransportTrip, type TransportTripDraft } from "@bookingapp/domain";
...
interface PublicTicketRow { route_name: string; mode: PublicTransportTicket["mode"]; origin_stop_id: string; destination_stop_id: string; quantity: number; seat_labels?: readonly string[] | null; reservation_status: ReservationStatus; status: TransportTicket["status"]; fare_amount_minor: number; fare_currency: string; issued_at: Date; boarding_starts_at: Date; boarding_ends_at: Date; }
interface PublicLiveTripRow { tenant_id: string; trip_id: string; route_name: string; captured_at: Date | null; latitude: number | null; longitude: number | null; accuracy_metres: number | null; heading_degrees: number | null; }
interface PublicLiveProjection { tripId: string; routeLabel: string; capturedAt: string | null; freshness: "live" | "delayed" | "signal_weak" | "offline"; latitude: number | null; longitude: number | null; accuracyMetres: number | null; headingDegrees: number | null; eta: null; }
...
export async function readPublicTransportLiveTrip(executor: SqlExecutor, token: string, secret: string): Promise<{ tenantId: string; projection: PublicLiveProjection } | null> {
  if (!token || !secret) return null;
  const rows = await executor.query<PublicLiveTripRow>("SELECT tt.tenant_id, tt.trip_id, r.name AS route_name, current.captured_at, current.latitude, current.longitude, current.accuracy_metres, current.heading_degrees FROM transport_tickets tt JOIN transport_trip_reservations tr ON tr.tenant_id = tt.tenant_id AND tr.reservation_id = tt.reservation_id AND tr.trip_id = tt.trip_id JOIN service_reservations sr ON sr.tenant_id = tr.tenant_id AND sr.id = tr.reservation_id JOIN transport_trips trip ON trip.tenant_id = tt.tenant_id AND trip.id = tt.trip_id JOIN transport_routes r ON r.tenant_id = trip.tenant_id AND r.id = trip.route_id AND r.version = trip.route_version LEFT JOIN fleet_tracking_sessions session ON session.tenant_id = tt.tenant_id AND session.trip_id = tt.trip_id AND session.status = 'active' AND session.expires_at > now() LEFT JOIN fleet_current_positions current ON current.tenant_id = session.tenant_id AND current.session_id = session.id WHERE tt.ticket_token_hash = $1 AND tt.status = 'issued' AND sr.status IN ('confirmed', 'checked_in') LIMIT 1", [tokenHash(token)]);
  const row = rows[0]; if (!row) return null;
  return { tenantId: row.tenant_id, projection: { tripId: row.trip_id, routeLabel: row.route_name, capturedAt: row.captured_at?.toISOString() ?? null, freshness: row.captured_at ? classifyPositionFreshness(row.captured_at, new Date()) : "offline", latitude: row.latitude, longitude: row.longitude, accuracyMetres: row.accuracy_metres, headingDegrees: row.heading_degrees, eta: null } };
}
