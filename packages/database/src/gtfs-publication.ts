...
import { isValidGtfsPublicId, type GtfsPublishedReferences, type GtfsScheduleFeature } from "@bookingapp/domain";
...
export async function recordGtfsValidation(executor: SqlExecutor, input: {
  ...
  scheduleSha256?: string; scheduleObjectKey?: string; scheduleReferences?: GtfsPublishedReferences; actorId?: string | null;
}): Promise<GtfsFeedVersion> {
  ...
  if (input.scheduleReferences) {
    await executor.query("DELETE FROM gtfs_feed_version_entities WHERE tenant_id = $1 AND feed_version_id = $2", [input.tenantId, input.feedVersionId]);
    for (const [entityKind, publicIds] of [["route", input.scheduleReferences.routeIds], ["trip", input.scheduleReferences.tripIds], ["stop", input.scheduleReferences.stopIds]] as const) {
      for (const publicId of publicIds) await executor.query("INSERT INTO gtfs_feed_version_entities (tenant_id, feed_version_id, entity_kind, public_id) VALUES ($1,$2,$3,$4)", [input.tenantId, input.feedVersionId, entityKind, publicId]);
    }
  }
  await appendAuditEvent(executor, { tenantId: input.tenantId, actorType: input.actorId ? "user" : "system", actorId: input.actorId ?? null, action: "gtfs.feed_generated", entityType: "gtfs_feed_version", entityId: input.feedVersionId, metadata: { issue_count: input.issues.length, blocking_issue_count: input.issues.filter((issue) => issue.severity === "error").length, artifact_written: Boolean(input.scheduleObjectKey && input.scheduleSha256), snapshot_entity_count: input.scheduleReferences ? input.scheduleReferences.routeIds.size + input.scheduleReferences.tripIds.size + input.scheduleReferences.stopIds.size : 0 } });
  return mapVersion(rows[0]);
}
