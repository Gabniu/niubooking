...
export async function endFleetTrackingSession(executor: SqlExecutor, input: { tenantId: string; sessionId: string; actorId?: string; reason: string }): Promise<FleetTrackingSession> {
  const rows = await executor.query<SessionRow>(`UPDATE fleet_tracking_sessions SET status = 'ended', ended_at = now(), ended_by = $3, end_reason = $4 WHERE tenant_id = $1 AND id = $2 AND status = 'active' RETURNING ${sessionColumns}`, [input.tenantId, input.sessionId, input.actorId ?? null, input.reason.slice(0, 200)]);
...
export async function ingestFleetPosition(executor: SqlExecutor, input: { tenantId: string; position: VehiclePosition }): Promise<TelemetryReceipt> {
  ...
  const sessions = await executor.query<SessionRow>(`SELECT ${sessionColumns} FROM fleet_tracking_sessions WHERE tenant_id = $1 AND id = $2 AND device_id = $3 FOR UPDATE`, [input.tenantId, input.position.sessionId, input.position.deviceId]);
